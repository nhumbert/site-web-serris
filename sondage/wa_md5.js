function IS_submitPagePassword(pwd,error_msg)
{
	...
	{
		var l=document.getElementById("is-password-form-layer-label")
		l.innerHTML=error_msg;	
...
{
	html_SetDisplay("is-global-layer",true)
	html_SetVisibility("is-global-layer",true);

	if (document.webaca_page_is_centered)
	{
		var l2 =document.getElementById("global-flash-page-centered")
		if (l2)
		{
			html_SetDisplay(l2,true)
			html_SetVisibility(l2,true);			
		}
	}
	html_SetVisibility("is-password-form-layer",false);
	html_SetDisplay("is-password-form-layer",false);

	WA_SetCookie("cariboost-password",pwd);
	IS_onload();
}

function IS_checkPagePassword(pwd)
{
	if (WA_GetCookie('cariboost-password')==pwd)
	{
		IS_successPagePassword(pwd);
		return true;
	}
	...
	{
		html_SetVisibility("is-password-form-layer",true);
		document.forms['is-password-form'].is_page_password.focus();
	}
	return false;
}
